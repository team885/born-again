...
const T={bg:"#F0F4FF",card:"#FFFFFF",card2:"#EEF2FF",border:"#C7D7FD",primary:"#2563EB",purple:"#6366F1",cyan:"#0EA5E9",green:"#059669",amber:"#D97706",red:"#DC2626",text:"#0F172A",sub:"#334155",muted:"#64748B"};

const USERS=[
  {id:"meron",name:"Meron",color:"#2563EB",initials:"ME"},
  {id:"aman",name:"Aman",color:"#0EA5E9",initials:"AM"},
  {id:"gideon",name:"Gideon",color:"#6366F1",initials:"GI"},
];
...
function AnimatedBackground(){
  return(
    <div style={{position:"fixed",inset:0,zIndex:0,overflow:"hidden",pointerEvents:"none"}}>
      <div style={{position:"absolute",width:700,height:700,borderRadius:"50%",background:"radial-gradient(circle,#2563EB12 0%,transparent 70%)",top:"-150px",left:"-150px",animation:"orbFloat1 20s ease-in-out infinite"}}/>
      <div style={{position:"absolute",width:600,height:600,borderRadius:"50%",background:"radial-gradient(circle,#6366F110 0%,transparent 70%)",bottom:"-50px",right:"-100px",animation:"orbFloat2 25s ease-in-out infinite"}}/>
      <div style={{position:"absolute",width:450,height:450,borderRadius:"50%",background:"radial-gradient(circle,#0EA5E90C 0%,transparent 70%)",top:"35%",left:"45%",animation:"orbFloat3 18s ease-in-out infinite"}}/>
    </div>
  );
}
...
function NotifyFeezModal({user,allData,allGoals,allNotes,onClose}){
  const td=allData[user.id]?.[todayKey()]||EMPTY_METRICS;
  const goals=allGoals[user.id]||DEFAULT_GOALS;
  const notes=(allNotes[user.id]||{})[todayKey()]||"";
  const rates=calcRates(td);
  const dateStr=new Date().toLocaleDateString("en-US",{weekday:"long",month:"long",day:"numeric",year:"numeric"});

  const goalLine=(label,actual,target)=>{
    const pct=target>0?Math.round((actual/target)*100):0;
    const hit=actual>=target;
    return `${label}: ${actual} / ${target} (${pct}%)${hit?" ✅":" 🔄"}`;
  };

  const emailBody=`Hi Feez,

Here are my stats for today — ${dateStr}:

━━━━━━━━━━━━━━━━━━━━━━━
📊 DAILY SUMMARY — ${user.name.toUpperCase()}
━━━━━━━━━━━━━━━━━━━━━━━

📞 ACTIVITY
• Calls Made: ${td.calls}
• Owners Spoken To: ${td.ownersSpoken}
• Leads in Pipeline: ${td.leadsInPipeline}

📅 APPOINTMENTS
• Booked: ${td.apptsBooked}
• Completed: ${td.apptsCompleted}
• No-Shows: ${td.apptsNoShow}

💰 DEALS
• Closed: ${td.dealsClosed}
• Lost: ${td.dealsLost}

━━━━━━━━━━━━━━━━━━━━━━━
📈 CONVERSION RATES
━━━━━━━━━━━━━━━━━━━━━━━
• Owner Contact Rate: ${rates.ownerRate}%
• Booking Rate: ${rates.bookingRate}%
• Show-Up Rate: ${rates.showRate}%
• Close Rate: ${rates.closeRate}%

━━━━━━━━━━━━━━━━━━━━━━━
🎯 GOALS VS ACTUALS
━━━━━━━━━━━━━━━━━━━━━━━
• ${goalLine("Calls",td.calls,goals.calls)}
• ${goalLine("Owners Spoken",td.ownersSpoken,goals.ownersSpoken)}
• ${goalLine("Appointments Booked",td.apptsBooked,goals.apptsBooked)}
• ${goalLine("Deals Closed",td.dealsClosed,goals.dealsClosed)}
...
Born Again Agency`;
  ...
  const copy=()=>{
    navigator.clipboard.writeText(emailBody).then(()=>{setCopied(true);setTimeout(()=>setCopied(false),2000);});
  };
  const openMail=()=>{
    const subject=encodeURIComponent(`Daily Stats — ${user.name} — ${new Date().toLocaleDateString("en-US",{month:"short",day:"numeric",year:"numeric"})}`);
    const body=encodeURIComponent(emailBody);
    window.open(`mailto:?subject=${subject}&body=${body}`);
  };

  return(
    <div onClick={e=>e.target===e.currentTarget&&onClose()} style={{position:"fixed",inset:0,background:"rgba(15,23,42,0.6)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:300,backdropFilter:"blur(8px)"}}>
      <div style={{background:T.card,border:`1px solid ${T.border}`,borderRadius:16,width:580,maxHeight:"88vh",display:"flex",flexDirection:"column",overflow:"hidden",boxShadow:"0 24px 80px rgba(37,99,235,0.15)"}}>
        <div style={{padding:"24px 28px 20px",borderBottom:`1px solid ${T.border}`,display:"flex",justifyContent:"space-between",alignItems:"flex-start",flexShrink:0}}>
          <div>
            <div style={{fontFamily:"'IBM Plex Mono',monospace",fontSize:9,color:T.primary,letterSpacing:"0.2em",marginBottom:4}}>NOTIFY FEEZ</div>
            <div style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:26,color:T.text,letterSpacing:"0.06em"}}>Daily Stats Report</div>
            <div style={{fontFamily:"'IBM Plex Mono',monospace",fontSize:9,color:T.muted,marginTop:2}}>{dateStr}</div>
          </div>
          <button onClick={onClose} style={{background:"transparent",border:"none",color:T.muted,cursor:"pointer",fontSize:20,lineHeight:1,padding:4}}>✕</button>
        </div>
        <div style={{flex:1,overflowY:"auto",padding:"20px 28px"}}>
          <pre style={{fontFamily:"'IBM Plex Mono',monospace",fontSize:11,color:T.text,lineHeight:1.8,whiteSpace:"pre-wrap",background:T.card2,border:`1px solid ${T.border}`,borderRadius:10,padding:"20px 24px",margin:0}}>{emailBody}</pre>
        </div>
        <div style={{padding:"16px 28px",borderTop:`1px solid ${T.border}`,display:"flex",gap:10,flexShrink:0}}>
          <button onClick={openMail} style={{flex:1,background:`linear-gradient(135deg,${T.primary},${T.purple})`,color:"white",border:"none",borderRadius:10,padding:"13px 0",fontFamily:"'Bebas Neue',sans-serif",fontSize:17,letterSpacing:"0.1em",cursor:"pointer",boxShadow:`0 4px 20px ${T.primary}33`}}>
            ✉ OPEN IN EMAIL
          </button>
          <button onClick={copy} style={{background:copied?T.green+"18":"transparent",color:copied?T.green:T.muted,border:`1px solid ${copied?T.green+"55":T.border}`,borderRadius:10,padding:"13px 20px",fontFamily:"'IBM Plex Mono',monospace",fontSize:11,cursor:"pointer",transition:"all 0.2s",letterSpacing:"0.1em"}}>
            {copied?"✓ COPIED":"COPY"}
          </button>
        </div>
      </div>
    ...
    <div style={{display:"flex",minHeight:"100vh",background:T.bg,color:T.text,position:"relative"}}>
      <style>{`
        ...
        *{box-sizing:border-box;margin:0;padding:0;}
        ...
        textarea{color:${T.text};background:${T.card};}
      `}</style>

      <AnimatedBackground/>

      {/* SIDEBAR */}
      <div style={{width:220,flexShrink:0,background:T.card+"F8",borderRight:`1px solid ${T.border}`,display:"flex",flexDirection:"column",position:"sticky",top:0,height:"100vh",zIndex:40,backdropFilter:"blur(12px)"}}>
      ...
      <button onClick={()=>setShowNotify(true)}
        onMouseEnter={e=>{e.currentTarget.style.transform="scale(1.05)";e.currentTarget.style.boxShadow=`0 12px 40px ${T.primary}44`;}}
        onMouseLeave={e=>{e.currentTarget.style.transform="scale(1)";e.currentTarget.style.boxShadow=`0 4px 20px ${T.primary}22`;}}
        style={{position:"fixed",bottom:80,right:28,background:`linear-gradient(135deg,${T.primary},${T.purple})`,color:"white",border:"none",borderRadius:14,padding:"12px 20px",fontFamily:"'Bebas Neue',sans-serif",fontSize:15,letterSpacing:"0.1em",cursor:"pointer",boxShadow:`0 4px 20px ${T.primary}22`,transition:"all 0.2s",zIndex:50,display:"flex",alignItems:"center",gap:8}}>
        ✉ NOTIFY FEEZ
      </button>
      {/* Calc FAB */}
      ...
      {showNotify&&<NotifyFeezModal user={user} allData={allData} allGoals={allGoals} allNotes={allNotes} onClose={()=>setShowNotify(false)}/>}
